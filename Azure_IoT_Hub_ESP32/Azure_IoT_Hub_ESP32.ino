...
static unsigned long next_telemetry_send_time_ms = 15000;
static char telemetry_topic[128];
static char telemetry_payload[100];
...
static void sendTelemetry() {
  ...
  size_t payload_size;
  int telemetry_created = generate_telemetry_payload(data_buffer, DATA_BUFFER_SIZE, &payload_size);

  Logger.Info("Telemetry variable");
  Logger.Info(telemetry_payload);
  Logger.Info("Telemetry Topic");
  ...
  if (az_result_failed(az_iot_hub_client_telemetry_get_publish_topic(
    ...
    if (esp_mqtt_client_publish(
          mqtt_client,
          telemetry_topic,
          (char*)az_span_ptr(az_span_create(data_buffer, payload_size)),
          az_span_size(az_span_create(data_buffer, payload_size)),
          MQTT_QOS1,
          DO_NOT_RETAIN_MSG)
        == 0) {
